...
static void format_timespec(time_t sec, long nsec, char *buf, size_t bufsz) {
    ...
    snprintf(buf, bufsz, "%s.%03ld", timestr, nsec / 1000000L);
}
...
static int run_writer(void) {
    ...
    while (1) {
        ...
        if (n < 0) strncpy(msg, "format-error", sizeof(msg));
        ...
        memcpy((void*)g_shm_ptr->message, msg, MSG_MAX-1);
